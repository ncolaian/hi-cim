...
hist(no_test_dist_vs_counts_tads$reads[no_test_dist_vs_counts_tads$distance == 1], breaks=300, xlim=range(0,2000), freq = FALSE)
par(new=TRUE)
hist(no_test_dist_vs_counts_tads$reads, breaks=100, xlim=c(600,1000))
plot(c(2,2,2,2), c(1,2,3,4))
(dist<-fitdistr(as.integer(no_tad_or_loops$reads[no_tad_or_loops$distance == (1)]),densfun = "poisson"))
BIC(dist)
#Poisson fit test
lamb <- trunc(mean(no_test_dist_vs_counts_tads$reads[no_test_dist_vs_counts_tads$distance == (1)]))
length(no_tad_or_loops$reads[no_tad_or_loops$distance == (1)])
tab1<- table(no_tad_or_loops$reads[no_tad_or_loops$distance == 1])
test <- rpois(length(no_test_dist_vs_counts_tads$reads[no_test_dist_vs_counts_tads$distance == (1)]), lamb)
test2 <- rnegbin(length(no_test_dist_vs_counts_tads$reads[no_test_dist_vs_counts_tads$distance == (1)]), mu = 942.9, theta = 31.5)
test3 <- rgamma(length(no_test_dist_vs_counts_tads$reads[no_test_dist_vs_counts_tads$distance == (1)]), 30.66, rate = 0.03251)
t <- hist(test, breaks=300, xlim=range(0,1500), freq = FALSE )
t2 <- hist(test2, breaks=300, xlim=range(0,2000), freq = FALSE, col="red" )
t3 <- hist(test3, breaks=300, xlim=range(0,2000), freq = FALSE )
set.seed(333)
nbin <- fitdistr(na.omit(as.integer(no_test_dist_vs_counts_tads$reads[no_test_dist_vs_counts_tads$distance == (1)])), "negative binomial")
po <- fitdistr(na.omit(as.integer(no_test_dist_vs_counts_tads$reads[no_test_dist_vs_counts_tads$distance == (1)])), "weibull")
gam <- fitdistr(na.omit(as.integer(no_test_dist_vs_counts_tads$reads[no_test_dist_vs_counts_tads$distance == (1)])), "gamma", lower=c(0,0))
nbin$estimate
gam$estimate
po$loglik

real <- hist(trunc_1, breaks = 300)
